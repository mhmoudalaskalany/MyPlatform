...
export class SessionManager {
  ...
  public IsAdmin(): boolean {
    ...
    if (roles !== undefined && roles !== null) {
      const role = roles.find((x) => x.RoleCode === 'FINANCIAL-SYSTEM-ADMINISTRATOR');
  ...
  private MapRoles(roles: any): void {
    ...
    const appRoles = parsedRoles.filter(x => x.AppCode === 'FINANCIAL-SYSTEM');
  ...
  private MapPermissions(permissions?: any[]) {
    const appPermission = permissions.find(x => x.appCode === 'FINANCIAL-SYSTEM');

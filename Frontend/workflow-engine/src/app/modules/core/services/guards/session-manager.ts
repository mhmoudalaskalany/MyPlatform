...
export class SessionManager {
  ...
  public IsAdmin(): boolean {
    ...
    if (roles !== undefined && roles !== null) {
      const role = roles.find((x) => x.RoleCode === 'SELF-SERVICES-ADMINISTRATOR');
  ...
  private MapRoles(roles: any): void {
    ...
    const appRoles = parsedRoles.filter(x => x.AppCode === 'SELF-SERVICES');
  ...
  private MapPermissions(permissions?: any[]) {
    const appPermission = permissions.find(x => x.appCode === 'SELF-SERVICES');

...
export class SessionManager {
  ...
  public IsAdmin(): boolean {
    ...
    if (roles !== undefined && roles !== null) {
      const role = roles.find((x) => x.RoleCode === 'IDENTITY-ADMINISTRATOR');
  ...
  private MapRoles(roles: any): void {
    ...
    const appRoles = parsedRoles.filter(x => x.AppCode === 'IDENTITY');
  ...
  private MapPermissions(permissions?: any[]) {
    const appPermission = permissions.find(x => x.appCode === 'IDENTITY');
